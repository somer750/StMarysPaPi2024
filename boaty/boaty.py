...
device = "BoatyMcHappyFace01" # set the device name, needs to be different for each device
...
def add_data(student): # function to read the button state and enter database record
    reject = False
    RGBG.on() # common anode, so this switches the light off (green = ready)
    RGBR.off() # common anode, so this switches the light off (red = wait)
...
try:
    while True:
        RGBG.off() # common anode, so this switches the light on (green = ready)
        RGBR.on() # common anode, so this switches the light off
